...
def send_admin_notification(name, email, subject, message, user_status, created_at):
    ...
    html_body = render_template(
        "emails/send_admin_notification.html",
        name=name,
        email=email,
        subject=subject,
        message=message,
    ...
    msg = Message(
        subject="New Cookfolio support message",
    ...
    try:
        mail.send(msg)
        current_app.logger.info("Notification sent to admin.")
        return True
...
# Email helper function to send confirmation email to the
# user who submitted the support message
...
def send_user_support_confirmation_email(name, email, subject, message, created_at):
    ...
    # Render the email template with the provided details
    html_body = render_template(
        "emails/send_user_support_confirmation.html",
        name=name,
        subject=subject,
        message=message,
        created_at=created_at,
    ...
    # Create the email message with the provided details
    msg = Message(
        subject="Cookfolio Support Confirmation",
        recipients=[email],
        body=f"""
        Hello {name},
    ...
    try:
        mail.send(msg)
        current_app.logger.info("Confirmation email sent to user:{name}")
        return True
    # Catch any exceptions that occur during email sending and log the error
    except Exception as e:
