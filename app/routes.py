...
def admin_view_support_messages():
    ...
    return render_template(
        "admin_support_messages.html", support_messages=support_messages
    )
...
def admin_view_support_message_details(message_id):
    ...
    return render_template(
        "admin_support_message_details.html", support_message=support_message
    )
...
def register():
    if request.method == "POST":
        ...
        send_welcome_email(name=new_user.username, email=new_user.email)
        ...
        if user:
            ...
            msg = Message(
                ...
                html=render_template(
                    "emails/reset_password_link.html", user=user, reset_link=reset_link
                ),
            )
            mail.send(msg)
            flash(
    ...
    if request.method == "POST":
        ...
        if photo and photo.filename:
            if allowed_file(photo.filename):
                ...
                flash(
                    "Invalid file type. Allowed types are png, jpg, jpeg, webp.",
                    "error",
                )
                return render_template(
                    "recipe_form.html",
        ...
        return redirect(
            url_for("main.view_recipe", recipe_id=new_recipe.id, next=return_url)
        )
...
def dashboard():
    ...
    # Join MealPlan with Recipe to get recipe details in
    # the same query and order by meal date and meal type
    weekly_meal_plans = (
    ...
    meal_plans = (
        MealPlan.query.filter(
            MealPlan.user_id == current_user.id,
            MealPlan.meal_date >= preview_start,
            MealPlan.meal_date <= preview_end,
        )
        .join(Recipe)
        .order_by(MealPlan.meal_date.asc())
        .all()
    ...
    for i in range(3):
        ...
        day_meals = [
            meal_plan for meal_plan in meal_plans if meal_plan.meal_date == preview_date
        ]
...
def view_recipe(recipe_id):
    ...
    return render_template(
        ...
        recipe=recipe,
        is_api_recipe=False,
        is_imported_recipe=is_imported_recipe,
    )
...
def edit_recipe(recipe_id):
    recipe = db.session.get(Recipe, recipe_id)
    return_url = request.args.get("next") or url_for(
        "main.view_recipe", recipe_id=recipe_id
    )
    ...
    if request.method == "POST":
        ...
        return redirect(
            url_for("main.view_recipe", recipe_id=recipe_id, next=return_url)
        )
...
def my_recipes():
    ...
    recipe_titles = (
        Recipe.query.with_entities(Recipe.title)
...
def meal_plans():
    ...
    meal_plans = (
        MealPlan.query.filter(
            MealPlan.user_id == current_user.id,
            MealPlan.meal_date >= start_of_week,
            MealPlan.meal_date <= end_of_week,
        )
        .join(Recipe)
    ...
    has_meal_plans = (
        MealPlan.query.filter_by(user_id=current_user.id).first() is not None
    )
    ...
    meal_plans = sorted(
        meal_plans,
        key=lambda mp: (mp.meal_date, meal_category_order.get(mp.meal_type, 99)),
    )
...
def add_meal_plan():
    recipes = (
        Recipe.query.filter_by(user_id=current_user.id).order_by(Recipe.title).all()
    )
    ...
    if not recipes:
        flash(
            "You need to have at least one recipe to add a meal plan."
    ...
    if request.method == "POST":
        ...
        existing_meal_category = MealPlan.query.filter_by(
            user_id=current_user.id, meal_date=meal_date, meal_type=meal_type
        ).first()

        if existing_meal_category:
            flash(
                (
                    f"You already have a {meal_type} planned for"
                    f"{meal_date.strftime('%d.%m.%Y')}."
                    f"Please edit it instead."
                ),
                "warning",
            )
            return render_template(
        ...
        # prevent same recipe been planned more
        # than once on the same day
        existing_recipe_for_day = MealPlan.query.filter_by(
            user_id=current_user.id, meal_date=meal_date, recipe_id=int(recipe_id)
        ).first()

        if existing_recipe_for_day:
            flash(
                (
                    f"You already have this recipe planned for"
                    f"{meal_date.strftime('%d.%m.%Y')}."
                    f"Please choose a different recipe."
                ),
                "warning",
            )
            return render_template(
        ...
        return redirect(
            url_for("main.view_day_meal_plan", meal_date=meal_date.strftime("%Y-%m-%d"))
        )
...
def view_day_meal_plan(meal_date):
    ...
    meal_plan = MealPlan.query.filter_by(
        user_id=current_user.id, meal_date=meal_date
    ).first()
    selected_date = datetime.strptime(meal_date, "%Y-%m-%d").date()
    day_meals = (
        MealPlan.query.filter_by(user_id=current_user.id, meal_date=selected_date)
        .join(Recipe)
    ...
    # Join MealPlan with Recipe to get recipe details
    # in the same query and order by meal type
    day_meals = (
        MealPlan.query.filter_by(user_id=current_user.id, meal_date=selected_date)
        .join(Recipe)
        .order_by(MealPlan.meal_type)
    ...
    day_meals = sorted(
        day_meals,
        key=lambda meal_plan: {
            "breakfast": 1,
            "lunch": 2,
            "dinner": 3,
            "dessert/snack": 4,
        }.get(meal_plan.meal_type, 99),
    )
...
def edit_meal_plan(meal_plan_id):
    ...
    return_url = request.args.get("next") or url_for(
        "main.view_day_meal_plan", meal_date=meal_plan.meal_date.strftime("%Y-%m-%d")
    )
    ...
    if request.method == "POST":
        ...
        try:
            meal_date = datetime.strptime(
                request.form.get("meal_date"), "%Y-%m-%d"
            ).date()
        except ValueError:
        ...
        if existing_meal_category and existing_meal_category.id != meal_plan_id:
            flash(
                (
                    f"You already have a {meal_type} planned for"
                    f"{meal_date.strftime('%d.%m.%Y')}."
                    f"Please choose a different meal type or edit the existing one."
                ),
                "warning",
            )
            return render_template(
        ...
        if existing_recipe_for_day and existing_recipe_for_day.id != meal_plan_id:
            flash(
                (
                    f"You already have this recipe planned for {meal_date}."
                    f"Please choose a different recipe."
                ),
                "warning",
            )
...
# Helper function to search recipes from TheMealDB and
# Spoonacular APIs by title, ingredients and instructions
...
def search_api_recipes(search_query, api_page=1):
    ...
    f"query={search_query}&number={api_per_page}"
    f"&apiKey={os.getenv('SPOONACULAR_API_KEY')}"
    spoonacular_params = {
        "query": search_query,
        "number": api_per_page,
        "offset": offset,
    ...
    spoonacular_response = requests.get(
        spoonacular_url, params=spoonacular_params, timeout=5
    )
...
# Quick daily planner form to add or update multiple
# meal plans for the same day in one form,
# with pre-filled existing meal plans for the selected day,
# and validation to prevent duplicates and past dates
...
def quick_add_meal_plan():
    ...
    return_url = request.args.get("next") or url_for(
        "main.view_day_meal_plan", meal_date=today
    )

    if not recipes:
        flash(
            "You need to have at least one recipe to use the quick add feature."
            "Please add a recipe first.",
            "error",
        )
        return redirect(url_for("main.meal_plans"))

    # Convert selected_date string to date object and validate
    # that it's in the correct format, if not set it to today
    try:
    ...
    # Get existing meal plans for the selected date to pre-fill the form,
    # and create a dictionary with meal_type as key for easy access in the template
    existing_meal_plan = MealPlan.query.filter_by(
        user_id=current_user.id, meal_date=selected_date_object
    ).all()
    ...
    if request.method == "POST":
        ...
        existing_meal_plan = MealPlan.query.filter_by(
            user_id=current_user.id, meal_date=meal_date_object
        ).all()
        meal_plan_data = {meal.meal_type: meal.recipe_id for meal in existing_meal_plan}
        ...
        if not has_selected_recipe:
            flash(
                "Please select at least one recipe to add or update meal plans.",
                "error",
            )
            return render_template(
                "quick_meal_plan_form.html",
        ...
        if len(recipe_ids) != len(set(recipe_ids)):
            flash(
                "Each recipe can only be selected once per day."
                "Please choose different recipes.",
                "error",
            )
            return render_template(
                "quick_meal_plan_form.html",
        ...
        # Initialize a counter to track how
        # many meal plans were added, updated or deleted
        added_or_updated = 0

        # Create or update meal plans accordingly
        for meal_type, recipe_id in selected_recipes.items():
            excisting_meal_plan = MealPlan.query.filter_by(
                user_id=current_user.id, meal_date=meal_date_object, meal_type=meal_type
            ).first()

            # If a recipe is selected for the meal type,
            # create or update the meal plan.
            # If no recipe is selected and a meal plan exists,
            # delete it.
            if recipe_id:
                # If a meal plan already exists for the meal type,
        ...
        # If no meal plans were added, updated or deleted,
        # show a warning message and stay on the form page
        if added_or_updated == 0:
            flash(
                "No meal plans were added or updated."
        ...
        # Show a success message indicating whether meal plans were added or updated,
        # and redirect to the day meal plan view
        ...
        return redirect(
            url_for(
                "main.view_day_meal_plan",
                meal_date=meal_date_object.strftime("%Y-%m-%d"),
            )
        )
...
# Search route to search recipe globally-
# users recipes and API recipes,
# search by title, ingredients and instructions,
# show results in a separate page with pagination
...
def search():
    ...
    if category_response.status_code == 200:
        ...
        api_categories = [
            category["strCategory"] for category in category_data.get("categories", [])
        ]
    ...
    # If a category is selected, filter meals by that category,
    # otherwise search by query or show all meals
    if selected_category:
        api_search_query = selected_category
    elif search_query:
        api_search_query = search_query
    else:
        # Default search query to show some results when search is empty
        api_search_query = "chicken"
    ...
    # search filter
    if search_query:
        user_query = Recipe.query.filter(
            db.or_(
    ...
    # category filter
    if selected_category:
        user_query = user_query.join(Category).filter(
            Category.name == selected_category
        )

    # pagination for user recipes, 8 recipes per page, newest first
    user_recipes = user_query.order_by(Recipe.created_at.desc()).paginate(
    ...
    if api_total_pages == 0:
        ...
        return redirect(
            url_for(
                "main.search", search=search_query, page=page, api_page=api_total_pages
            )
        )
...
# Discover route to show recipes from the API with
# category filter, search and pagination
...
def view_discover_recipe(meal_id):
    ...
    if current_user.is_authenticated:
        already_imported = Recipe.query.filter_by(
            user_id=current_user.id, title=meal.get("strMeal")
        ).first()
...
def import_discover_recipe(meal_id):
    ...
    if existing_recipe:
        ...
        # if user want to add recipe to planner before importing,
        # redirect to planner page, otherwise redirect to recipe details page
    ...
    # otherwise redirect to recipe details page
    if redirect_to_planner:
...
def view_spoonacular_recipe(recipe_id):
    ...
    recipe = {
        ...
        "description": (
            data.get("cuisines")[0] if data.get("cuisines") else "Unknown" + " recipe"
        ),
...
def import_spoonacular_recipe(recipe_id):
    api_url = f"https://api.spoonacular.com/recipes/{recipe_id}"
    f"/information?apiKey={os.getenv('SPOONACULAR_API_KEY')}"
    response = requests.get(api_url)
    redirect_to_planner = request.args.get("planner")
    ...
    category = Category.query.filter_by(
        name=category_mapping.get(data.get("dishTypes")[0], "Dinner")
    ).first()
...
def support():
    if request.method == "POST":
        ...
        send_admin_notification(
            ...
            user_status=(
                support_message.user_id if support_message.user_id else "Guest User"
            ),
...
def grocery_list():
    ...
    # Convert selected_date string to date object and
    # validate that it's in the correct format, if not set it to today
    if selected_date:
    ...
    # Loop through meal plans and extract ingredients from the associated recipes,
    # categorizing them by meal type
    ...
    meal_type_order = {"breakfast": 1, "lunch": 2, "dinner": 3, "dessert/snack": 4}
    ...
    return render_template(
        "grocery_list.html",
        grocery_items=ordered_grocery_items,
        selected_date=selected_date,
