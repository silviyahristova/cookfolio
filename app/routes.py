...
def admin_view_support_messages():
    ...
    support_messages = SupportMessage.query.order_by(
        SupportMessage.created_at.desc()).paginate(page=page, per_page=5, error_out=False)
...
def reset_password(token):
    ...
    if request.method == 'POST':
        ...
        if check_password_hash(user.password, password):
            flash(
                'This password has been used before. Please choose a new password.', 'error')
            return redirect(url_for('main.reset_password', token=token))
    ...
    if request.method == 'POST':
        ...
        if photo and photo.filename:
            if allowed_file(photo.filename):
                ...
                flash(
                    'Invalid file type. Allowed types are png, jpg, jpeg, webp.', 'error')
                return render_template('recipe_form.html', form_type='add', categories=categories, return_url=return_url, form_data=request.form)
...
def dashboard():
    ...
    weekly_meal_plans = MealPlan.query.filter(MealPlan.user_id == current_user.id, MealPlan.meal_date >=
                                              week_start, MealPlan.meal_date <= week_end).join(Recipe).order_by(MealPlan.meal_date.asc()).all()

    # Get meal plans for the preview days and order by meal date and meal type
    meal_plans = MealPlan.query.filter(MealPlan.user_id == current_user.id, MealPlan.meal_date >= preview_start,
                                       MealPlan.meal_date <= preview_end).join(Recipe).order_by(MealPlan.meal_date.asc()).all()
            ...
            if allowed_file(photo.filename):
                ...
                if recipe.image_filename:
                    ...
                    old_image_path = os.path.join(
                        upload_folder, recipe.image_filename)
...
def my_recipes():
    ...
    recipe_titles = Recipe.query.with_entities(Recipe.title).filter_by(
        user_id=current_user.id).order_by(Recipe.title).all()
...
def meal_plans():
    ...
    meal_plans = MealPlan.query.filter(MealPlan.user_id == current_user.id, MealPlan.meal_date >= start_of_week,
                                       MealPlan.meal_date <= end_of_week).join(Recipe).order_by(MealPlan.meal_date, MealPlan.meal_type).all()
    has_recipes = Recipe.query.filter_by(
        user_id=current_user.id).first() is not None
    has_meal_plans = MealPlan.query.filter_by(
        user_id=current_user.id).first() is not None
    ...
    meal_plans = sorted(meal_plans, key=lambda mp: (
        mp.meal_date, meal_category_order.get(mp.meal_type, 99)))
...
def add_meal_plan():
    ...
    if request.method == 'POST':
        ...
        existing_meal_category = MealPlan.query.filter_by(
            user_id=current_user.id, meal_date=meal_date, meal_type=meal_type).first()

        if existing_meal_category:
            flash(
                f'You already have a {meal_type} planned for {meal_date.strftime("%d.%m.%Y")}. Please edit it instead.', 'warning')
            return render_template('meal_plan_form.html', recipes=recipes, meal_type=meal_type, selected_date=selected_date, return_url=return_url, form_data=request.form)

        # prevent same recipe been planned more than once on the same day
        existing_recipe_for_day = MealPlan.query.filter_by(
            user_id=current_user.id, meal_date=meal_date, recipe_id=int(recipe_id)).first()

        if existing_recipe_for_day:
            flash(
                f'You already have this recipe planned for {meal_date.strftime("%d.%m.%Y")}. Please choose a different recipe.', 'warning')
            return render_template('meal_plan_form.html', recipes=recipes, meal_type=meal_type, selected_date=selected_date, return_url=return_url, form_data=request.form)
...
def view_day_meal_plan(meal_date):
    ...
    meal_plan = MealPlan.query.filter_by(
        user_id=current_user.id, meal_date=meal_date).first()
    selected_date = datetime.strptime(meal_date, '%Y-%m-%d').date()
    day_meals = MealPlan.query.filter_by(user_id=current_user.id, meal_date=selected_date).join(
        Recipe).order_by(MealPlan.meal_type).all()
    ...
    day_meals = MealPlan.query.filter_by(user_id=current_user.id, meal_date=selected_date).join(
        Recipe).order_by(MealPlan.meal_type).all()
    day_meals = sorted(day_meals, key=lambda meal_plan: {
                       'breakfast': 1, 'lunch': 2, 'dinner': 3, 'dessert/snack': 4}.get(meal_plan.meal_type, 99))
...
def edit_meal_plan(meal_plan_id):
    ...
    if request.method == 'POST':
        ...
        meal_date = datetime.strptime(
            request.form.get('meal_date'), '%Y-%m-%d').date()
        ...
        if existing_meal_category and existing_meal_category.id != meal_plan_id:
            flash(
                f'You already have a {meal_type} planned for {meal_date.strftime("%d.%m.%Y")}. Please choose a different meal type or edit the existing one.', 'warning')
            return render_template('meal_plan_form.html', recipes=recipes, meal_plan=meal_plan, today=date.today(), selected_date=selected_date, return_url=return_url, form_data=request.form)
        ...
        if existing_recipe_for_day and existing_recipe_for_day.id != meal_plan_id:
            flash(
                f'You already have this recipe planned for {meal_date}. Please choose a different recipe.', 'warning')
            return render_template('meal_plan_form.html', recipes=recipes, meal_plan=meal_plan, today=date.today(), selected_date=selected_date, return_url=return_url, form_data=request.form)
...
def search_api_recipes(search_query, api_page=1):
    ...
    if spoonacular_response.status_code == 200:
        ...
        for recipe in spoonacular_recipes:
            ...
            recipe["cuisine"] = ",".join(recipe.get(
                "cuisines", [])) or "Spoonacular Recipe"
            recipe["title"] = recipe.get("title", "Untitled Recipe")
            recipe["image_url"] = recipe.get("image", "")
            recipe["category"] = recipe.get("dishTypes", ["Unknown"])[
                0] if recipe.get("dishTypes") else "Unknown"
...
def quick_add_meal_plan():
    ...
    existing_meal_plan = MealPlan.query.filter_by(
        user_id=current_user.id, meal_date=selected_date_object).all()
    meal_plan_data = {
        meal.meal_type: meal.recipe_id for meal in existing_meal_plan}
    ...
    if request.method == 'POST':
        ...
        existing_meal_plan = MealPlan.query.filter_by(
            user_id=current_user.id, meal_date=meal_date_object).all()
        meal_plan_data = {
            meal.meal_type: meal.recipe_id for meal in existing_meal_plan}
        ...
        has_selected_recipe = any(
            recipe_id for recipe_id in selected_recipes.values())

        if not has_selected_recipe:
            flash(
                'Please select at least one recipe to add or update meal plans.', 'error')
            return render_template('quick_meal_plan_form.html', recipes=recipes, today=today, meal_plan_data=meal_plan_data, selected_date=meal_date, return_url=return_url)

        # Prevent dupliate recipes for the same day
        recipe_ids = [
            recipe_id for recipe_id in selected_recipes.values() if recipe_id]
        if len(recipe_ids) != len(set(recipe_ids)):
            flash(
                'Each recipe can only be selected once per day. Please choose different recipes.', 'error')
            return render_template('quick_meal_plan_form.html', recipes=recipes, today=today, meal_plan_data=meal_plan_data, selected_date=meal_date, return_url=return_url)
        ...
        for meal_type, recipe_id in selected_recipes.items():
            excisting_meal_plan = MealPlan.query.filter_by(
                user_id=current_user.id, meal_date=meal_date_object, meal_type=meal_type).first()
        ...
        if added_or_updated == 0:
            flash(
                'No meal plans were added or updated. Please select at least one recipe.', 'warning')
            return render_template('quick_meal_plan_form.html', recipes=recipes, today=today, meal_plan_data=meal_plan_data, selected_date=meal_date, return_url=return_url)
...
def search():
    ...
    categories = Category.query.order_by(Category.order).all()
    selected_category = selected_category = request.args.get(
        'category', '').strip()
    page = request.args.get('page', 1, type=int)
    ...
    if category_response.status_code == 200:
        ...
        api_categories = [category['strCategory']
    ...
    # search filter
    if search_query:
        user_query = Recipe.query.filter(db.or_(Recipe.title.ilike(f'%{search_query}%'), Recipe.ingredients.ilike(
            f'%{search_query}%'), Recipe.instructions.ilike(f'%{search_query}%')))
...
def view_discover_recipe(meal_id):
    ...
    if current_user.is_authenticated:
        already_imported = Recipe.query.filter_by(
            user_id=current_user.id, title=meal.get('strMeal')).first()
...
def view_spoonacular_recipe(recipe_id):
    ...
    if current_user.is_authenticated:
        already_imported = Recipe.query.filter_by(
            user_id=current_user.id, title=data.get('title')).first()
...
def import_spoonacular_recipe(recipe_id):
    ...
    category = Category.query.filter_by(name=category_mapping.get(
        data.get('dishTypes')[0], 'Dinner')).first()
...
def support():
    if request.method == 'POST':
        ...
        send_admin_notification(
            ...
            created_at=support_message.created_at.strftime(
                '%H:%M:%S on %d.%m.%Y')
        )
