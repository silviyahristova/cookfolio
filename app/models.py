...
# Importing Mapped and relationship for defining model attributes and relationships between models
...
class User(db.Model, UserMixin):
    ...
    ), nullable=False)  # Timestamp for when the user account was created
    ...
    def generate_reset_token(self):
        # Create a serializer using the application's secret key for securely signing the token
        ...
        self.reset_token = token  # Store the generated token in the user's reset_token field
    ...
    def verify_reset_token(token, expiration=3600):
        ...
        try:
            # Attempt to decode the token and retrieve the email, ensuring it hasn't expired
        ...
        # Check if the user exists and if the stored reset token matches the provided token
        if user is None or user.reset_token != token:
            return None  # If the user doesn't exist or the tokens don't match, return None
...
class Category(db.Model):
    ...
    # Field to specify the order of categories in the dropdown menu
