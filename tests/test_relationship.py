...
def test_deleting_user_deletes_associated_recipes_and_support_messages(db_session, test_user):
    ...
    deleted_recipe = db_session.query(
        Recipe).filter_by(user_id=test_user).first()
    deleted_support_message = db_session.query(
        SupportMessage).filter_by(user_id=test_user).first()
