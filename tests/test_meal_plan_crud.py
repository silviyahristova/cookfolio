...
def test_add_meal_plan(logged_in_client, test_recipe):
    # Set meal date to 2 days in the future to avoid validation errors
    meal_date = datetime.now() + timedelta(days=2)
...
def test_edit_meal_plan(logged_in_client, test_meal_plan, test_recipe):
    ...
    # Set meal date to 3 days in the future to avoid validation errors
    meal_date = datetime.now() + timedelta(days=3)
    response = logged_in_client.post(
        f"/meal-plans/{meal_plan_id}/edit",
        data={
            "meal_date": meal_date.strftime("%Y-%m-%d"),
            "meal_type": "Lunch",
            "recipe_id": test_recipe,
...
def test_delete_meal_plan(logged_in_client, test_meal_plan):
    meal_plan_id = test_meal_plan
    response = logged_in_client.post(
        f"/meal-plans/{meal_plan_id}/delete", follow_redirects=True
    )
