...
def test_my_recipes_page_requires_login(
    app, test_user, test_recipe, client, logged_in_client
):
...
def test_my_recipes_search_only_finds_user_recipes(
    app, test_user, test_recipe, logged_in_client
):
    with app.test_client() as client:
        category = Category.query.filter_by(name="Breakfast").first()
        if not category:
        ...
        other_user = User(
            ...
            password=generate_password_hash("otherpassword"),
        )
...
def test_my_recipes_page_displays_user_recipes(
    client, test_user, test_recipe, logged_in_client
):
...
def test_my_recipes_add_recipe_button_position(
    client, test_user, test_recipe, logged_in_client
):
...
# test pagination shows correct number of recipes per
# page and pagination controls appear when there are more than 10 recipes
